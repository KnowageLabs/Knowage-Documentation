...
Figure 6.6: From left to right: (a) Generic configuration axis (the specific arrow). (b) Generic configuration axis.
...
To split the first series over the second one, remember that it is necessary to choose only one attribute as category field and two measures as series values. Figure 6.12 shows an example.

   |image104|

   Figure 6.12: Split over second series.

Meanwhile to split a measure over second category it is mandatory to choose exactly two attributes as category field and only one measure as series value, as shown in Figure 6.13.
...
Once you selected the dataset and the type of chart, choose at least two attributes in the X-axis panel and a measure in the Y-axis panel as showed in Figure 6.16.
...
In Figure 6.18 you find the sunburst obtained with data of Table 6.1.
   ...
   Figure 6.18: From left to right: (a) Sunburst. (b) Sunburst category.(c) Sunburst subcategory.
...
To create a parallel chart select a dataset with at least one attribute and two columns with numerical values. You can find an interesting example of dataset in Table 6.2 where we display some of its rows.

In this example three different classes of iris are studied. Combining the values of some sepal and petal width or lenght, we are able to find out which class we are looking at. In Figure 6.21(a) you can find the parallel chart made with the suggested dataset. While in Figure 6.21(b) it is easy to see, thanks to selection, that all iris with petal length between 2,5 and 5.2 cm and petal width 0,9 and 1,5 cm belong to the iris-versicolor class.
...
The next step is to move to **Configuration** tab and select the **Color palette** icon. Here (Figure 6.25) the user has to define the chromatic scale which will be associated to the measure values. The panel will demand the user to insert the first, the last color and the number of bands that will constitute the color scale.

The engine will create a progressive color scale as shown in the left image of Figure 6.26. To custom the scale the user can use the Preset colors and use the arrow to move up and down Heatmap chart the added color or the user can increase the number of steps and then
...
To give an idea of the outcome, we take as instance the Bar Chart drill down. In the following example, the selected categories are four and called: product_family, product_department, product_category and product_subcategory. Once we open the document, we get as shown in
Figure 6.28:
...
When selecting “shelf_depth” measure of the Food category one gets (see next Figure 6.29):

   |image122|

   Figure 6.29: Drillable Bar Chart: first drill

Once again, we can select “Frozen food” subcategory and drill to a second sub_level as in Figure 6.30:
