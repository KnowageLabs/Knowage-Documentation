...
-  (d) undo, • (e) delete versions, • (f) output wizard.
...
-  (d) save button,
   ...
   Slicing • (e) cancel button.

   |image154|

   Figure 7.20: Hierarchies dialog pop up.

   After selecting the hierarchy and saving user’s choice, that
   ...
   We give an example of the output when the hierarchy “Time” is
   selected in Figure 7.22 and hierarchy “Time Weekly” in Figure 7.23.
...
-  (g) a highlighted member (result of searching), • (h) a save and a
      cancel buttons.
...
2. by typing member’s name or it’s part in the input field and clicking
      on the search button. The research will be possible if the user
      enters at least four letters. If the user wishes to include
      member’s siblings to the research, the checkbox (Figure 7.24, (d))
      needs to be checked (Figure 7.26);

3. after the first research, if the user types some other member’s name
...
~~~~~~~~~
   ...
   on a button (see Figure 7.5) located on the right side of dimension’s
...
2. **Member**: if the user wants to perform drill operation not only on
   ...
   Figure 7.30: “Position” drill down. member drill type. See Figure
   7.31.
...
3. to click on the “Apply” button (after checking the checkbox, remember
   ...
   loaded data can be exported in csv format by clicking on the “Export”
   button.

Refreshing model
~~~~~~~~~~~~~~~~

   To refresh a loaded model the user needs to click on the “Refresh”
...
-  type a query in a text area of send MDX dialogs (Figure 7.36), •
   ...
   Result of the MDX query “should” appear in pivot table as in Figure
...
~~~~~~~~~~~~~~~~~~~~~~
   ...
   the pivot table (for example: member’s hierarchy, level or parent
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   ...
   “Suppress empty rows/colums” button in the side bar panel. An example
...
~~~~~~~
   ...
   To enable member ordering the user must click on the “Enable sorting”
   button in the side bar panel. The command for sorting will appear
   next to the member’s name in the pivot table. In addition, the
   sorting command will show the members of “Measures” hieararchy or
...
-  (f) a save button.
   ...
   If the user selects “Count sorting” mode the top or last 10 members
...
2. click on the “calculated field” button in the side bar panel: a
      “Select function” dialog will appear. The latter consists of
   ...
   tab doesn’t contain any formula, it will not be displayed. The “Name”
   field is mandatory, indeed the creation of a function without a name
   is forbidden. In **Recent tab**, the “Name” field is hidden for
...
3. Select a function and enter a calculated member/set name and click on
      “Ok”. A dialog for arguments defintion will show up ,as shown in
...
-  (f) open saved button, • (g) select from table button,
...
3. |image188|\ Import expression from saved calculated members or sets.
   ...
   • Click on open saved button. Then the dialog of saved calculated
...
-  After filling all the arguments of function, clicking on OK button
   ...
   In tab “Recent”, opening the “Select function” dialog the user can
